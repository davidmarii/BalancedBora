...
# + BACKGROUND TASKS — Instant WhatsApp replies, no timeouts
# + LRU CACHE — Repeat calculations in under 50ms
...
# ============================================================
# CACHED SOLVER — Repeat calculations in <50ms
# ============================================================
@lru_cache(maxsize=256)
def cached_solve_nrc_ration(profile_key: str, selected_feeds_tuple: tuple):
    """Hashable wrapper for solve_nrc_ration. Caches up to 256 recent combos."""
    return solve_nrc_ration(profile_key, list(selected_feeds_tuple))
# ============================================================
...
def format_ration(phone, result):
    ...
    return msg
# ============================================================
# BACKGROUND TASK — Runs LP solver after instant reply
# ============================================================
def process_ration_and_reply(phone: str, profile_key: str, feed_ids: list, lang: str):
    """Run LP solver in background and send result via Twilio."""
    start = time.time()

    # Ensure session has language set for get_msg
    if phone not in user_sessions:
        user_sessions[phone] = {}
    user_sessions[phone]['lang'] = lang

    # Use cached solver for instant repeat calculations
    result, error = cached_solve_nrc_ration(profile_key, tuple(sorted(feed_ids)))
    ...
    if error and isinstance(error, str) and error.startswith("❌"):
        ...
        reply_text = get_msg(phone, 'no_forage_error')
        ...
        reply_text = format_ration(phone, result)

    # Send the final answer back to the farmer
    if client:
        try:
...
# ============================================================
# WEBHOOK — INSTANT REPLY + BACKGROUND PROCESSING
# ============================================================
@app.post("/whatsapp")
async def whatsapp_webhook(
    ...
    if session.get('step') == 2 and 'ai_detected_feeds' in session and text in ['yes', 'yep', 'sawa', 'correct', 'ndio', 'ii']:
        ...
        # QUICK VALIDATION (fast, synchronous)
        ...
        # INSTANT REPLY — Twilio gets this in under 1 second
        msg.body(get_msg(phone, 'calculating'))
        session['step'] = 0

        # HEAVY MATH RUNS IN BACKGROUND
...
def health_check():
    return {
        ...
        "features": ["nrc_2001_lp", "best_effort_mode", "ai_suggestions", "image_recognition", "21_feeds", "native_translations", "background_tasks", "lru_cache"],
        "vision_configured": bool(GOOGLE_API_KEY),
        "sessions": len(user_sessions),
        "cache_info": str(cached_solve_nrc_ration.cache_info())
    }
